...
export function setCoupons(state, [loading, data = null]) {
  if (data) {
    state.coupons = {
      ...state.coupons,
      data: data.data,
...
// export function addCoupon(state, coupon) {
//   state.coupons.data.push(coupon);
// }

// export function updateCoupon(state, updatedCoupon) {
//   const index = state.coupons.data.findIndex((coupon) => coupon.id === updatedCoupon.id);
//   if (index !== -1) {
//     state.coupons.data[index] = updatedCoupon;
//   }
// }

// export function deleteCoupon(state, couponId) {
//   state.coupons.data = state.coupons.data.filter((coupon) => coupon.id !== couponId);
// }
