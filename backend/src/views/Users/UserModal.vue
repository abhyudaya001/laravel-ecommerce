                ...
                <footer class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                  <!-- <button type="submit"
                   ...
                   <button
                      ...
                      class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                    >
                      Submit
                    </button>
                  <button type="button"
...
// Add a computed property to validate the password
const passwordError = computed(() => {
  ...
  if (!passwordPattern.test(user.value.password)) {
    return 'Password must contain at least one number, one letter, and one symbol.'
  }
...
function onSubmit() {
  loading.value = true
  const passwordValidationError = passwordError.value
  if (passwordValidationError) {
    // If there is a password validation error, do not proceed with submission
    loading.value = false
    return
  }
