...
export function getCoupons({ commit, state }, { url = null, search = '', per_page, sort_field, sort_direction } = {}) {
  commit('setCoupons', [true])
  url = url || '/coupons' // Adjust the API endpoint for coupons
  const params = {
    per_page: state.coupons.limit, // Replace 'products' with 'coupons' here
  }
  return axiosClient.get(url, {
    ...
    .then((response) => {
      commit('setCoupons', [false, response.data])
    })
    .catch(() => {
...
export function getCoupon({ commit }, id) {
  return axiosClient.get(`/coupons/${id}`) // Adjust the API endpoint for a single coupon
}

export function createCoupon({ commit }, coupon) {
  // Adjust the API endpoint for creating a coupon
  return axiosClient.post('/coupons', coupon)
}

export function updateCoupon({ commit }, coupon) {
  const id = coupon.id
  // Adjust the API endpoint for updating a coupon
  return axiosClient.put(`/coupons/${id}`, coupon)
}

export function deleteCoupon({ commit }, id) {
  // Adjust the API endpoint for deleting a coupon
